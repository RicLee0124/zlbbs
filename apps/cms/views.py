...
def email_captcha():
    ...
    source = list(string.ascii_letters)  # 返回小写a~z的字母
    source.extend(map(lambda x: str(x), range(0, 10)))
    captcha = "".join(random.sample(source, 6))  # 生成验证码
...
# @bp.route('/email/')
# def send_mail():
#     message = Message('邮件发送', recipients=['18373229089@163.com'], body='测试')
#     mail.send(message)
#     return 'success'
...
@bp.route('/boards/')
@login_required
@permission_required(CMSPermission.BOARDR)
def boards():
