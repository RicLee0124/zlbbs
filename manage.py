...
def create_role():
    ...
    admin.permissions = CMSPermission.VISITOR | CMSPermission.POSTER | CMSPermission.CMSUSER | CMSPermission.COMMENTER | CMSPermission.FRONTUSER | CMSPermission.BOARDR

    # 4.开发者
    developer = CMSRole(name='开发者', desc='开发人员专有权限')
...
# python manage.py add_user_to_role -e 18373229089@163.com -n 访问者
@manager.option('-e', '--email', dest='email')
...
@manager.command
def test_permission():
    user = CMSUser.query.filter_by(username='admin').first()
